...
export const httpJsonApiServer = (port: string) => {
...
function handleParseTimeRequest(queryParams: URLSearchParams, res: http.ServerResponse<http.IncomingMessage>) {
  const isoValue = queryParams.get('iso');
  if (!isoValue) {
...
function handleUnixTimeRequest(queryParams: URLSearchParams, res: http.ServerResponse<http.IncomingMessage>) {
  const isoValue = queryParams.get('iso');
  if (!isoValue) {
    handleError(res, 400, 'ISO parameter is missing');
    return;
...
function handleError(res: http.ServerResponse<http.IncomingMessage>, statusCode: number, message: string) {
  res.statusCode = statusCode;
  res.setHeader('Content-Type', 'text/plain');
  res.end(message);
}

function sendJSONResponse(res: http.ServerResponse<http.IncomingMessage>, data: any) {
  res.setHeader('Content-Type', 'application/json');
